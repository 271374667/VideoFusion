...
class VideoRemover(BlackRemoveAlgorithm):
    def remove_black(self, input_file_path: str | Path) -> tuple[int, int, int, int]:
        video_path: Path = Path(input_file_path)
        loguru.logger.info(f'正在使用差值法检测视频变化区域: {video_path.name}')

        # 如果不是视频则报错
        if video_path.suffix not in ['.mp4', '.avi', '.flv', '.mov', '.mkv']:
            raise ValueError(f"文件不是视频: {video_path}")
...
if __name__ == '__main__':
    # 使用示例
    video_path = r"E:\load\python\Project\VideoFusion\tests\test_data\videos\001.mp4"
    video_remover = VideoRemover()
    max_rectangle = video_remover.remove_black(video_path)
